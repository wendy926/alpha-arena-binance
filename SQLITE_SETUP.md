# Alpha Arena SQLite版本设置指南

## 🚀 快速启动

### 方法1: 完整安装（推荐）
```bash
chmod +x install_all_deps.sh
./install_all_deps.sh
python3 init_sqlite.py
python3 web_server.py
```

### 方法2: 快速修复openai模块
```bash
chmod +x quick_fix_openai.sh
./quick_fix_openai.sh
python3 web_server.py
```

### 方法3: 一键修复脚本
```bash
chmod +x fix_sqlite_and_deps.sh
./fix_sqlite_and_deps.sh
```

### 方法4: 分步执行
```bash
# 1. 安装依赖
chmod +x install_deps.sh
./install_deps.sh

# 2. 修复数据库表结构
python3 check_db_schema.py

# 3. 初始化数据库
python3 init_sqlite.py

# 4. 启动web服务器
python3 web_server.py
```

## 🔧 问题解决

### 问题1: `table trades has no column named symbol`
**原因**: 旧的数据库表结构不匹配
**解决**: 运行 `python3 check_db_schema.py` 重建表结构

### 问题2: `ModuleNotFoundError: No module named 'flask'`
**原因**: 缺少Python依赖包
**解决**: 运行 `./install_deps.sh` 安装依赖

### 问题3: `ModuleNotFoundError: No module named 'openai'`
**原因**: 缺少openai模块
**解决**: 运行 `./quick_fix_openai.sh` 或 `./install_all_deps.sh`

### 问题4: 端口占用 `Address already in use`
**原因**: 端口8080已被其他进程占用
**解决方案**:

#### 方案1: 自动修复端口占用（推荐）
```bash
chmod +x fix_port_8080.sh
./fix_port_8080.sh
```

#### 方案2: 使用备用端口启动
```bash
chmod +x start_web_alternative.sh
./start_web_alternative.sh
```

#### 方案3: 手动指定端口
```bash
PORT=8081 python3 web_server.py
# 或者
PORT=9000 python3 web_server.py
```

#### 方案4: 手动检查和释放端口
```bash
# 查看占用端口8080的进程
lsof -i:8080

# 终止占用进程（替换PID为实际进程ID）
kill -9 PID
```

### 问题5: 权限问题
**解决**: 
```bash
chmod +x *.sh
```

## 📊 数据库信息

- **数据库类型**: SQLite
- **数据库文件**: `./data/paper_trades.db`
- **表结构**:
  - `trades`: 交易记录表
  - `positions`: 持仓记录表

## 🌐 访问地址

- **默认地址**: http://localhost:8080
- **VPS外网访问**: http://你的VPS_IP:8080
- **备用端口**: 如果8080被占用，可能使用8081、8082、9000等端口

**注意**: 确保VPS防火墙已开放相应端口

## 📝 配置文件

项目已配置使用SQLite，相关配置在 `.env` 文件中：
```
DB_TYPE=sqlite
SQLITE_DB_PATH=./data/paper_trades.db
```

## 🎯 优势

- ✅ 无需Docker容器
- ✅ 轻量级数据库
- ✅ 避免MySQL重启问题
- ✅ 数据持久化存储